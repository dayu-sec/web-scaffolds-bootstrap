...
# 微应用创建契约保持独立，避免与单体项目脚本共享实现后互相影响。
# 微应用模板地址属于脚本维护配置；使用公开 GitHub HTTPS 地址，无需额外认证配置。
...
readonly MICROAPP_NAME_FORMAT_RULE="必须使用 kebab-case，只能包含小写字母、数字和连字符"

MICROAPP_NAME=""
MICROAPP_DESCRIPTION=""
...
用法：
  create-microapp.sh
  create-microapp.sh --name <微应用名称> [选项]

交互模式：
  不传任何参数时，脚本会逐项询问微应用名称、模板分支、描述、目标目录、新仓库地址和 Git 历史选项。
...
可选参数：
  ...
  --repository <地址>        设置新微应用的 origin；未提供时不设置远端
  --target <目录>            克隆目标目录；默认是当前目录下的 <微应用名称>
                             传入 . 时直接使用当前目录
  --branch <分支>            指定模板仓库分支；默认使用仓库默认分支
  --keep-history             保留微应用模板仓库的 Git 历史
  -h, --help                 显示帮助

默认行为：
  未提供 --branch 时使用模板仓库默认分支。
  删除微应用模板仓库的 Git 历史并初始化新的 Git 仓库；提供 --keep-history 时保留历史。
...
validate_microapp_name() {
  MICROAPP_NAME_ERROR=""

  if [[ -z "$MICROAPP_NAME" ]]; then
    MICROAPP_NAME_ERROR="微应用名称不能为空。"
    return 1
  fi

  if [[ ${#MICROAPP_NAME} -gt $NAME_MAX_LENGTH ]]; then
    MICROAPP_NAME_ERROR="微应用名称不能超过 ${NAME_MAX_LENGTH} 个字符。"
    return 1
  fi

  if [[ ! "$MICROAPP_NAME" =~ ^[a-z0-9]+(-[a-z0-9]+)*$ ]]; then
    MICROAPP_NAME_ERROR="微应用名称${MICROAPP_NAME_FORMAT_RULE}。"
    return 1
  fi
...
validate_runtime_configuration() {
  [[ -n "$MICROAPP_TEMPLATE_REPOSITORY" ]] || fail "请先在脚本中配置 MICROAPP_TEMPLATE_REPOSITORY。"
...
通过管道非交互执行时，请显式传递参数，例如：
  curl -fsSL <脚本地址> | bash -s -- --name new-microapp
  wget -qO- <脚本地址> | bash -s -- --name new-microapp --repository <新仓库地址>
EOF
}
...
collect_interactive_arguments() {
  ...
  while true; do
    printf '\n微应用名称（必填）：\n规则：最多 %s 个字符；%s。\n> ' \
      "$NAME_MAX_LENGTH" "$MICROAPP_NAME_FORMAT_RULE" >&3
    read_from_terminal
    MICROAPP_NAME="$USER_INPUT"

    if validate_microapp_name; then
      break
    fi

    printf '输入无效：%s\n' "$MICROAPP_NAME_ERROR" >&3
  done
  ...
  if [[ -n "$USER_INPUT" ]]; then
    MICROAPP_DESCRIPTION="$USER_INPUT"
    DESCRIPTION_PROVIDED=true
  fi
...
prepare_package_json() {
  ...
  if [[ "$DESCRIPTION_PROVIDED" == true ]]; then
    ...
    jq --arg name "$MICROAPP_NAME" --arg description "$MICROAPP_DESCRIPTION" '
      [
        to_entries[]
    ...
    jq --arg name "$MICROAPP_NAME" '.name = $name' "$package_json" >"$PACKAGE_TEMP_FILE"
  fi
}
...
# 先清除模板远端或历史，再按需设置新 origin，避免新微应用继续指向微应用模板仓库。
...
clone_template() {
  if [[ -n "$TEMPLATE_BRANCH" ]]; then
    git clone --branch "$TEMPLATE_BRANCH" -- "$MICROAPP_TEMPLATE_REPOSITORY" "$TARGET_DIRECTORY"
  else
    git clone -- "$MICROAPP_TEMPLATE_REPOSITORY" "$TARGET_DIRECTORY"
  fi
}

# 微应用创建成功后仅在交互模式调用；依赖安装失败时由 set -e 阻止继续启动开发服务器。
prompt_and_run_pnpm() {
  while true; do
    ...
    case "$USER_INPUT" in
      "" | y | Y | yes | YES | Yes)
        command -v pnpm >/dev/null 2>&1 ||
          fail "未找到 pnpm；微应用已创建，但无法执行 pnpm install 和 pnpm dev。"
        printf '\n正在安装依赖...\n' >&3
        (
          cd "$TARGET_DIRECTORY"
          pnpm install
...
main() {
  if (($# == 0)); then
    ...
    parse_arguments "$@"
    validate_microapp_name || fail "$MICROAPP_NAME_ERROR"
    validate_runtime_configuration
  fi
